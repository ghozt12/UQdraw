...
$(document).ready(function () {
    ...
    var SubmissionInfo = React.createClass({
        //basically it shows dynamic data of number of responses and titles
        render: function () {
            return (
                <div>
                    <h2 className="type-body-heading">Correct answers for: {this.props.title}</h2>
                </div>
            );
        }
    });
    ...
    var SubmissionList = React.createClass({//print every response boxes
        //ListView contain mutiple submissions
        render: function () {
            var submissionNodes = this.props.subList.map(function (submission) {
                if (parseInt(submission.result) == 1) {
                    return (
    ...
    React.render(
        //pass data to SubmissionBox, it relresh every 2sec
        <ResultBox
